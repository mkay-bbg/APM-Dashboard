...
# Load trained model
saved_model = tf.keras.models.load_model('saved models (inflow)')
...
prediction = pd.DataFrame({"pred_ppl_in":real_y_hat.reshape(len(real_y_hat))}, index=df_train[train_size:].index)
...
class Prediction():
    ...
    def in_pred():
        fig_in = px.line(df_summary[["ppl_in", "pred_ppl_in"]], title="Prediction and raw data plot (People inflow)")
        fig_in.update_xaxes(title="Time")
        fig_in.update_yaxes(title="People inflow")
        return fig_in
